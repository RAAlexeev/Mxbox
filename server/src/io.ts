...
export function setDO(num:number,value:number){
   fs.writeFile('/sys/devices/virtual/misc/mtgpio/pin',`-w=${num}:0 0 0 ${value} 0 1 0`,(err)=>{if (err) console.error(err);})
}
...
export function ioInit(){
  for (const n in _di) {
   fs.writeFile('/sys/devices/virtual/misc/mtgpio/pin',`-w=${n}:0 0 0 0 0 0 0`,(err)=>{if (err) console.error(err);})
  }
